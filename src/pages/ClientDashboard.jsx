...
// If you store date="2025-01-01" and time="10:00" as *local IST time*,
// this helper converts it to a proper Date object in IST.
// Make sure backend uses the SAME logic (or TZ=Asia/Kolkata on server).
const parseBookingDateTime = (booking) => {
  let start = booking.startDateTime ? new Date(booking.startDateTime) : null;
  let end = booking.endDateTime ? new Date(booking.endDateTime) : null;

  if (!start && booking.date && booking.time) {
    const iso = `${booking.date}T${booking.time}:00+05:30`; // IST
    start = new Date(iso);
  }
...
const canJoinBookingNow = (booking, now = new Date()) => {
  const { start, end } = parseBookingDateTime(booking);
...
const splitBookingsByTime = (bookings) => {
  const now = new Date();
  const upcoming = [];
  const completed = [];

  bookings.forEach((b) => {
    const { start, end } = parseBookingDateTime(b);

    if (!start || !end) {
      // if no proper times, treat as upcoming
      upcoming.push(b);
      return;
    }
...
const formatBookingDate = (b) => {
  if (b.date) return new Date(b.date).toLocaleDateString();
  if (b.startDateTime) return new Date(b.startDateTime).toLocaleDateString();
  return "";
};
...
const ClientDashboard = ({ user }) => {
  ...
  const [pendingBookings, setPendingBookings] = useState([]); // kept if you need later
        ...
        setPreviewImage(
          p.profileImage
            ? p.profileImage.startsWith("http")
              ? p.profileImage
              : `https://healpeer-backend.onrender.com${p.profileImage}`
            : null
        );
      } catch (err) {
        console.error("Failed to fetch profile:", err);
  ...
  // Reset pagination when data sizes change
  useEffect(() => {
    setCompletedPage(1);
  }, [bookings.length]);
  ...
  const { upcoming: upcomingBookings, completed: completedBookings } = useMemo(
    () => splitBookingsByTime(bookings),
    [bookings]
  );
  ...
  const nextSession = useMemo(
    () =>
      upcomingBookings.length
        ? // sort upcoming by start time and take first
          [...upcomingBookings].sort((a, b) => {
            const { start: sa } = parseBookingDateTime(a);
            const { start: sb } = parseBookingDateTime(b);
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-slate-800 flex flex-wrap items-center gap-2">
                        ...
                        {formatBookingDate(nextSession) && (
                          <>
                            <span className="text-slate-400">•</span>
                            <span>
                              {formatBookingDate(nextSession)} at{" "}
                              {nextSession.time}
                            </span>
                          </>
                        )}
                ...
                >
                  {upcomingBookings.length === 0 ? (
                    ...
                    upcomingBookings.map((b) => {
                      ...
                      return (
                        <Card key={b._id || b.id} className="p-4 md:p-5">
                          <div className="flex flex-wrap items-start justify-between gap-4">
                            <div className="flex gap-3">
                              <div
                                className="w-11 h-11 rounded-xl bg-slate-100 flex items-center justify-center cursor-pointer"
